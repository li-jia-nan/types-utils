...
> = T extends `${infer Current}&${infer Rest}`
  ? Current extends `${infer K}=${infer V}`
    ? InternalUrlQueryToObject<Rest, InternalAddUrlQueryValueToResult<Result, K, V>>
    : InternalUrlQueryToObject<Rest, InternalAddUrlQueryValueToResult<Result, Current, undefined>>
 ...
 * ```
 */
export type UrlQueryToObject<T extends string> = T extends `${string}?${infer QueryString}`
  ? InternalUrlQueryToObject<QueryString>
  : InternalUrlQueryToObject<T>;

type InternalUrlParamsToUnion<T extends string> = T extends `${infer Current}/${infer Rest}`
 ...
 * ```
 */
export type UrlParamsToUnion<T extends string> = T extends `${infer Pathname}?${string}`
