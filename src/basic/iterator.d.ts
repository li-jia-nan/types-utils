 ...
 * ```
 */
export type MapEntry<T extends ReadonlyMap<unknown, unknown>> = T extends ReadonlyMap<
  infer K,
  infer V
>
  ? [K, V]
 ...
 * ```
 */
export type SetEntry<T extends ReadonlySet<unknown>> = T extends ReadonlySet<infer U>
  ? [U, U]
  : never;
 ...
 * ```
 */
export type Entries<T> = Entry<T> extends infer E ? (IsNever<E> extends true ? never : E[]) : never;
